...
public class Game : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (gameState)
        {
            ...
            case GAMESTATE.ADJUST:
                fetchInformation();
                AdjustPosition();
                if (int.Parse(p1_Score.text) - int.Parse(p2_Score.text) >= 2)
                {
                    btn_p2_Illusion.interactable = true;
                    btn_p2_Illusion.image.color = Color.red;
                }
                else if (int.Parse(p2_Score.text) - int.Parse(p1_Score.text) >= 2)
                {
                    btn_p1_Illusion.interactable = true;
                    btn_p1_Illusion.image.color = Color.red;
                }
    ...
    private void EnterHittingAnimation()
    {
        float back = 1f;
        float front = 4f;
        Vector3 direction = (cursorPosition - whiteBallPosition).normalized;
        direction.y = 0;
